...
module "platform_backend_api_prod" {
  ...
  backend_id                  = "${local.project_folder_code}-backend-api-prod"
  subdomains                  = ["api"]
  dns_managed_zone_name       = "dnszone-${local.project_folder_code}-mktskillsai"
  dns_managed_zone_project_id = local.dns_zone_project_id
  serverless_backends = [{
    name           = "${local.project_folder_code}-backend-api-prod"
    region         = local.env_main_region
    cloud_run_name = "crunserv-${local.project_folder_code}-backend-api-prod"
  }]
